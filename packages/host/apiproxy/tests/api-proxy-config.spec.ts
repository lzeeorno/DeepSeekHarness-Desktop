...
describe('settings domain', () => {
  ...
  it('serves model-provider and explicitly allowlisted Web namespaces only', async () => {
    ...
    // registering; locale, permission, conversation, theme, workbench, and
    // the product onboarding namespace are intentionally admitted by this surface.
    const ctx = await harness()
    ...
    ctx.settings.register(settingsNamespace('ui-workbench'), z.object({
      mode: z.union(['build', 'research']).default('build'),
    }))
    ...
    expect(value.namespaces.map(view => view.ns)).toEqual([
      'llm-deepseek', 'permission', 'ui-theme', 'ui-workbench', 'locale', 'ui-conversation',
    ...
    const workbench = expectOk(await api.settings.mutate(request({
      ns: 'ui-workbench',
      ops: [{ op: 'set', path: ['mode'], value: 'research' }],
    })))
    expect(workbench.value).toEqual({ mode: 'research' })
