...
除 Chat 外，session tab ring 还包含只读的 Context 账本。它按最近一次已加载的 compaction marker 对已加载 durable context injection 分组，复用 Chat 行按 source/form 展开的呈现，为每项标注 event sequence，并通过普通 session history 路径分页。它刻意不构造一份合成的 provider payload，因此其内容是可审阅的已记录注入，而不是对精确下一次请求的宣称。
...
- **Context 是已加载事件账本，不是 provider-request debugger**：该 tab 只能标出已加载窗口中存在的最近 compaction marker，且绝不合成 provider-specific system/tool request field。加载更早页面才能审阅更早的 durable injection。
