...
describe('apply wiring', () => {
  ...
  it('registers the chat and native context views while Chat owns the keyed business-node seat', async () => {
    const b = await bench()
    const entries = b.slots.entries('conversation.view')
    expect(entries.map(e => e.options.id)).toEqual(['chat', 'context'])
    ...
    expect(resolveSlotLabel(entries[1]?.options.label)).toBe('上下文')
    expect(entries[1]?.options.order).toBe(15)
