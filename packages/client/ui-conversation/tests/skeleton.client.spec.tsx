...
function mount(
  ...
  const renderSlot = ((key: string, owner: object, opts?: { only?: string }) => {
    ...
    if (key === 'conversation.session.header.actions') headerActionOwner = owner
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hands header actions the current session view selector', () => {
    const b = mount(conversationSnapshot())
    const owner = b.headerActionOwner() as { selectView: (viewId: string) => void }
    act(() => { owner.selectView('trajectory') })
