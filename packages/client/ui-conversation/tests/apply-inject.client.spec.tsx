...
import type { ContextLedgerViewInjected } from '../src/client/chat/ContextLedgerView.tsx'
...
async function bench() {
  ...
  /** Same session-scoped resolution for the native context ledger entry. */
  const contextViewApi = (id: SessionId) => {
    const entry = runtime.slots.entries('conversation.view').find(value => value.options.id === 'context')!
    const injected = (entry.inject as unknown as (sessionId: SessionId) => ContextLedgerViewInjected)(id)
    return { injected }
  }
  ...
  return {
    ...
    conversationApi, conversationHeaderApi, residentApi, composerApi, chatViewApi, contextViewApi, inputApi,
...
describe('conversation slot inject API', () => {
  it('assembles the thin API side-effect-free', async () => {
    ...
    expect(injected.views.list().map(v => v.id)).toEqual(['chat', 'context'])

    const chatView = b.chatViewApi(ROOT)
    chatView.injected.loadOlder()
    expect(b.sessionFake.loadOlder).toHaveBeenCalledTimes(1)
    b.contextViewApi(ROOT).injected.loadOlder()
    expect(b.sessionFake.loadOlder).toHaveBeenCalledTimes(2)
  ...
  it('views read face projects the ring ledger (subscribe/version through ctx.slots)', async () => {
    ...
    expect(injected.views.list().map(v => v.id)).toEqual(['chat', 'chat2', 'context'])
    ...
    expect(injected.views.list().map(v => v.label)).toEqual(['对话', 'X', 'bare', '上下文'])
    off()
    off2()
    unsub()
    await b.runtime.dispose()
