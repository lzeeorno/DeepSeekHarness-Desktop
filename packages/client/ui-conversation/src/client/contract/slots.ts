...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
     ...
     * passes the current session's view selector. Controls otherwise read
     * their state from the framework session kit (`sessionId`, `useSession`,
     * `useInput`, `inputActions`) and their own inject face.
     */
    ...
    /**
     * Build/Research lens selector on the new-session screen. Root scope: it
     * is a local UI preference rather than a property of a session or prompt.
     */
    'conversation.hero.workbench': { kind: 'single'; scope: 'root'; owner: HeroWorkbenchOwnerProps }
...
/** Owner share of the hero workbench selector: the shell supplies nothing. */
export interface HeroWorkbenchOwnerProps {
  /** Marker field: the selector owns its own persisted UI preference. */
  children?: never
}
...
/** Header action currency supplied by the owning session chrome. */
export interface ConversationHeaderActionOwnerProps {
  /** Select a registered session view; unknown ids retain the Chat fallback. */
  selectView: (viewId: string) => void
}

/** Header utilities derive their state from the standard session/global kit. */
