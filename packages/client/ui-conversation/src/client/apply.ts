...
import { ContextLedgerView, type ContextLedgerViewInjected } from './chat/ContextLedgerView.tsx'
...
export function apply(ctx: Context): void {
  ...
  slots.register({
    ...
    children: {
      ...
      'conversation.hero.workbench': { kind: 'single', scope: 'root' },
    },
  ...
  // The context ledger belongs beside Chat because this package owns the
  // durable context-node projection and its established source/body renderer.
  slots.register({
    ...
    locale: NS,
    inject: (sessionId: SessionId): ContextLedgerViewInjected => {
      const scoped = scopedConversation(sessions, sessionId)
      return { loadOlder: () => { void scoped.loadOlder() } }
    },
  }, ContextLedgerView)
