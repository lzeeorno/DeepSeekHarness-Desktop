...
describe('healProfilesModuleFallback', () => {
  ...
  it('follows a symlinked bundle to its transitive dependency', () => {
    const root = tmp()
    const appDir = join(root, 'app')
    const bundleDir = join(root, 'store', 'bundle-a')
    const dependencyDir = join(bundleDir, 'node_modules', 'dep-of-a')
    mkdirSync(join(appDir, 'node_modules'), { recursive: true })
    mkdirSync(dependencyDir, { recursive: true })
    writeFileSync(join(appDir, 'package.json'), JSON.stringify({
      name: 'dsh-app', dependencies: { 'bundle-a': '0.0.0' },
    }))
    writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
      name: 'bundle-a', dependencies: { 'dep-of-a': '0.0.0' },
    }))
    writeFileSync(join(dependencyDir, 'package.json'), JSON.stringify({ name: 'dep-of-a' }))
    symlinkSync(bundleDir, join(appDir, 'node_modules', 'bundle-a'), 'junction')

    const home = tmp()
    healProfilesModuleFallback(join(appDir, 'package.json'), home)

    expect(readlinkSync(join(home, 'profiles', 'node_modules', 'dep-of-a'))).toBe(dependencyDir)
  })
