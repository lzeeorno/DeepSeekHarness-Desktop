...
describe('web e2e: Models settings page configures a dormant provider', () => {
  ...
  it('materializes OpenAI and Anthropic BYOK profiles as separate write-only credentials', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-models-openai-anthropic'))
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.getByRole('button', { name: '模型' }).click()

    for (const { provider, credential, reference } of [
      { provider: 'openai', credential: 'sk-e2e-openai', reference: 'OPENAI_API_KEY' },
      { provider: 'anthropic', credential: 'sk-e2e-anthropic', reference: 'ANTHROPIC_API_KEY' },
    ]) {
      await dialog.getByRole('button', { name: '添加提供方' }).click()
      await dialog.getByLabel('提供方').selectOption(provider)
      await dialog.getByRole('textbox', { name: 'API 密钥', exact: true }).fill(credential)
      await dialog.getByRole('button', { name: '保存', exact: true }).click()
      await dialog.getByText(`已保存 ${provider}。`, { exact: true }).waitFor({ timeout: 10_000 })
      await expect.poll(
        () => scaffold.ctx.llm.listProviders().some(entry => entry.id === provider),
        { timeout: 10_000 },
      ).toBe(true)
      ...
      expect(settings).toContain(`apiKeyEnv: ${reference}`)
      expect(settings).not.toContain(credential)
      expect(credentials).toContain(`${reference}: ${credential}`)
