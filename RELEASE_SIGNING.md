...
For a signed macOS build, the release environment must provide a Developer ID Application certificate through `CSC_LINK` and `CSC_KEY_PASSWORD`, and may select it with `CSC_NAME`. Notarization uses one complete tuple: `APPLE_API_KEY`, `APPLE_API_KEY_ID`, and `APPLE_API_ISSUER`; or `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, and `APPLE_TEAM_ID`; or an `APPLE_KEYCHAIN_PROFILE`. These values belong in the CI secret store and must never be committed or pasted into a prompt.
